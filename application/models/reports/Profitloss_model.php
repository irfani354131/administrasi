...
class Profitloss_model extends CI_Model {
	...
	function get_data_akun_biaya_other() {
		$query = $this->db->query("SELECT * FROM `acc_coa_type` WHERE `deleted` = 0 AND `account_type` in('Beban Lain-lain') AND parent is NULL ORDER BY account_number ASC");
		if($query->num_rows() > 0) {
			$out = $query->result();
			return $out;
		} else {
