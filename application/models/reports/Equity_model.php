...
class Equity_model extends CI_Model {
	...
	function get_data_ekuitas() {
		$query = $this->db->query("SELECT * FROM `acc_coa_type` WHERE `deleted` = 0 AND account_type = 'Ekuitas' AND parent IS NULL ORDER BY account_number ASC");
		if($query->num_rows() > 0) {
			$out = $query->result();
			return $out;
		} else {
	...
	function get_data_prive() {
		$query = $this->db->query("SELECT * FROM `acc_coa_type` WHERE `deleted` = 0 AND id = 135 AND parent IS NULL ORDER BY account_number ASC");
		if($query->num_rows() > 0) {
			$out = $query->result();
			return $out;
		} else {
	...
	function get_data_akun_biaya() {
		$query = $this->db->query("SELECT * FROM `acc_coa_type` WHERE `deleted` = 0 AND `account_type` in('Beban') AND parent is NULL ORDER BY account_number ASC");
		if($query->num_rows() > 0) {
			$out = $query->result();
			return $out;
		} else {
	...
	function get_modal_akun_month($akun,$month,$year,$project=false,$akun_number=false) {
		$start=$year.'-'.$month.'-01';
		$end=$year.'-'.$month.'-31';
			...
			$this->db->join('acc_coa_type','master_saldo_awal.fid_coa=acc_coa_type.id');
			$this->db->where('periode', ''.$year.'');
			$this->db->where('DATE(date) <= ', ''.$end.'');
			$this->db->where('master_saldo_awal.deleted ', '0');
			$this->db->where_in('fid_coa', $total_akun);
