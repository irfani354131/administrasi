...
class Salary extends MY_Controller {
    ...
    function index() {
		...
		$data['judul']="Salary";
        $this->template->rander("salary/index",$data);
    }
    ...
    function postingjurnal($id){
        ...
        if(!empty($id)){
			...
			$data_sal_cat = array( 
				'sa_tanggal_posting' => get_current_utc_time(),
				'sa_status' => 2,
			); 
			$this->Salary_model->update_salary_cat($data_sal_cat,$id);
			...
			*/
			...
			$cek_data=$this->Salary_model->salary_data($id);
			
			$data_posting1 = array( 
				'date' => get_current_utc_time(),
				'type' => 'jurnal_umum',
				'description' => $cek_data->sa_keterangan,
				'fid_coa' => 115,
				'fid_header' => 0,
				'debet' => $cek_data->sa_total, 
				'credit' => 0, 
				'created_at' => get_current_utc_time(),
			); 
			$this->Salary_model->save_transaction($data_posting1);
			sleep(1);
			$data_posting2 = array( 
				'date' => get_current_utc_time(),
				'type' => 'jurnal_umum',
				...
				'fid_header' => 0,
				'debet' => 0, 
				'credit' => $cek_data->sa_total, 
				'created_at' => date("Y-m-d H:i:s"),
			); 
			$this->Salary_model->save_transaction($data_posting2);
			//redirect
            //echo json_encode(array("success" => true,"data" => $id));
			redirect(base_url().'employees/salary/view/'.$id);
        }else{
			redirect(base_url().'employees/salary/view/'.$id);
            //echo json_encode(array('success' => false,'message' => lang('error_occurred')));
        }
    ...
    function modal_form() {
        ...
        $this->load->view('salary/modal_form',$view_data);
    }
    ...
    function gajiupdate() {
		...
		$ket_penambah = $this->input->post('ket_penambah');
		$pengurang = $this->input->post('pengurang');
		$ket_pengurang = $this->input->post('ket_pengurang');
		...
		//$stdGet = "<ul>"; //Untuk deskripsi pada invoice
		if($ada==1){
			...
			foreach ($pegawai as $key => $val) {
					if($val!=""){
						$data_sal = array(  
							...
							'tahun' => $tahun,
							'gaji' => $gaji[$key],
							'nominal_pengurang' => $pengurang[$key],
							'deskripsi_pengurang' => $ket_pengurang[$key],
							'nominal_penambah' => $penambah[$key],
							'deskripsi_penambah' => $ket_penambah[$key],
							'gaji_diterima' => $total[$key],
							'fid_salary' => $fid, 

						); 
						
						$this->Salary_model->update_salary($data_sal,$id_salary[$key]);
			...
			foreach ($pegawai as $key => $val) {
					if($val!=""){
						$data_sal = array( 
							'kode'    => getCodeId('salary_items',"SI"),
							'pegawai_id'    => $val,
							'bulan'  => $bulan,
							'tahun' => $tahun,
							'gaji' => $gaji[$key],
							...
							'gaji_diterima' => $total[$key],
							'fid_salary' => $fid,
							'created_time' => get_current_utc_time(),

						); 
						
						$this->Salary_model->save_salary($data_sal);
		...
		$data_sal_cat = array( 
			'sa_status' => 1,
			'sa_total' => array_sum($total),
			'sa_user_tanggal' => get_current_utc_time(),//lastupdate 
		); 
		
		$this->Salary_model->update_salary_cat($data_sal_cat,$fid);
		...
		if ($fid) { 
            //echo json_encode(array("success" => true, "salary_id" => $fid, "data" => '', 'id' => $fid, 'message' => lang('record_saved')));
			//echo "<script";
			redirect(base_url().'employees/salary/view/'.$fid);
    ...
    /* add or edit an item */
     ...
     function add() {
        ...
        $item_data = array(
			"code" => getCodeId('salary',"SL"),  
            "sa_bulan" => $this->input->post('sal_bulan'),
            "sa_tahun" => $this->input->post('sal_tahun'),
            "sa_keterangan" => $this->input->post('deskripsi'),
			"sa_createdby" => $this->session->userdata('user_name'),
            "sa_createdate" => get_current_utc_time(), 
            "sa_tanggal" => date("Y-m-d"),
            "deleted" =>0,
        );
		//print_r($item_data);
        
		$data = $this->Salary_model->save($item_data);
        if ($data) {
            $item_info = $this->Salary_model->get_maxi()->row();
    ...
    function view($id= 0,$hal=FALSE){
        ...
        if ($id) {
				//echo $id;
            $view_data = get_detail_salary($id);
			//$bulan=$hal;
			//print_r($view_data['data_info']);
			$vd=$view_data['data_info'];
			//$bulan=$vd->sa_bulan;
			$idne=$vd->id;
			//print_r($view_data);
            if ($view_data) {
				$periode = $this->Salary_model->get_data_periode($idne);
				$ada=1;
				$hitperiode=count($periode);
				if($hitperiode<1){
					$periode = $this->Salary_model->get_data_pegawai();	
					$ada=0;
				}
				
                $view_data['ada'] = $ada;
                $view_data['gaji'] = $periode;
                ...
                $this->template->rander("salary/view", $view_data);
            } else {
                show_404();
            }
        }else{
    ...
    function list_data() {
        ...
        $list_data = $this->Salary_model->get_details()->result();
		//$this->db->error();
        $result = array();
        $before='';
    ...
    private function _make_item_row($data,$before) {
		...
		$row_data[] = anchor(get_uri("employees/salary/view/".$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_item'), "data-post-id" => $data->id));
  ...
  //              . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => "Delete Customers", "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("master/customers/delete"), "data-action" => "delete"));
    ...
    function get_item_suggestion_pengeluaran() {
        ...
        foreach ($items as $item) {
            $suggestion[] = array("id" => $item->title, "text" => $item->title, "price" => '', "category" => 'Sparepart',"unit_type" => '');
        }
