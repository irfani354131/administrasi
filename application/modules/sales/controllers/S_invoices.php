...
class S_invoices extends MY_Controller {
    ...
    private function _make_row($data) {
        ...
        $row_data = array(
            ...
            $query->code,
            modal_anchor(get_uri("master/customers/view/" . $data->fid_cust), 
            $query->name, array("class" => "view", "title" => "Customers ".$query->name, "data-post-id" => $data->fid_cust)),
        ...
        if($data->status != "posting" AND $data->is_verified == "0"){
           $row_data[] = anchor(get_uri("sales/s_invoices/edit/".$data->id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => "Edit Invoices"))
    ...
    function save_item() {
		...
		//Ambil Data Produksi
		$get_dt_produksi=$this->db->query('select * from produksi_barangjadi where id="'.$kode.'" AND deleted=0')->row();
		$jumlahBarangdiGudang=$get_dt_produksi->bj_qty-$quantity;
		$jumlahBarangKeluarGudang=$get_dt_produksi->bj_qtykeluar+$quantity;
		...
		$updAdd=$this->Sales_InvoicesItems_model->_update_data_produksi($dataupds,$kode);
    ...
    private function _make_item_row($data) {
        ...
        if($val->status != "posting" AND $val->is_verified == "0"){
                    return array(
                        ...
                        .js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("sales/s_invoices/delete_item"), "data-action" => "delete")),
            $item,
    ...
    function get_item_info_suggestion() {
        //$item = $this->Sales_InvoicesItems_model->get_item_info_suggestion($this->input->post("item_name"));//edited
        $item = $this->Sales_InvoicesItems_model->get_item_info_suggestion_join($this->input->post("item_name"));
        if ($item) {
