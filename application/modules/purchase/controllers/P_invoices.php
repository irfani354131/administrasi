...
class P_invoices extends MY_Controller {
    ...
    function modal_form() {
        ...
        $view_data['quot_dropdown'] = array("" => "-") + $this->Purchase_Request_model->get_dropdown_quot(array("code"));
    ...
    function add() {
        ...
        $order_id = $this->input->post('fid_quot');
        ...
        if (!empty($order_id)) {
            ...
            $check = $this->db->query("SELECT * FROM purchase_request_items WHERE fid_quotation = '$order_id' AND deleted = 0")->result();
    ...
    function posting_save() {
        ...
        try{
            ...
            $coa_persediaan = 145;   // Persediaan Barang deposit tiket
            $coa_hutang_usaha = 3; // Hutang Usaha IDR
            $coa_dp_beli = 15;
            ...
            if ($save_id) {
				...
				//Start Script Tambahan Proses  penambahan ke Kuantitas Barang
				$dataCop=$this->Purchase_InvoicesItems_model->get_item_add($id);
					
					foreach($dataCop as $dc){
						$kodeproduk=$dc->kode;
						$jumlah=$dc->quantity;
						$rests = substr($kodeproduk, 0, 2); // returns "d"
						//echo $rests;
						if($rests=="SP"){
							$dataAdd=$this->Purchase_InvoicesItems_model->get_item_sparepart($kodeproduk);
							$jumlahDasar=$dataAdd->kuantitas_pembelian;
							//get_item_sparepart
							$jmUpdProduk=$jumlahDasar+$jumlah;
							
							        $dataupd = array(
										"kuantitas_pembelian" => $jmUpdProduk,
									);
							
							$updAdd=$this->Purchase_InvoicesItems_model->_update_item_sparepart($dataupd,$kodeproduk);
								
						}
						
						if($rests=="PM"){
							
							$dataAdd=$this->Purchase_InvoicesItems_model->get_item_material($kodeproduk);
							//get_item_sparepart
							$jumlahDasar=$dataAdd->pm_kuantitas_pembelian;
							$jmUpdProduk=$jumlahDasar+$jumlah;
							
							        $dataupd = array(
										"pm_kuantitas_pembelian" => $jmUpdProduk,
									);
							
							$updAdd=$this->Purchase_InvoicesItems_model->_update_item_material($dataupd,$kodeproduk);
						}
						
							
					}
					//End Script Tambahan Proses  penambahan ke Kuantitas Barang
				//var_dump($this);
    ...
    private function _make_row($data) {
        ...
        $row_data = array(
            ...
            modal_anchor(get_uri("master/vendors/view/" . $data->fid_cust), $query->name, array("class" => "view", "title" => "Vendors ".$query->name, "data-post-id" => $data->fid_cust)),
    ...
    function save_item() {
        ...
        $invoice_item_id = $this->Purchase_InvoicesItems_model->save($invoice_item_data, $id);
			...
			$kode=$this->input->post('kode_produk');
			$harga=unformat_currency($this->input->post('invoice_item_rate'));
			...
			//print_r($dtSparePt);
			if(count($dtSparePt)>0){
				$jmlBarangDBPembelian=$dtSparePt->kuantitas_pembelian; 
				$jmlBarangDBTersedia=$dtSparePt->saldo_perlengkapan; 
				$hargaBarang=$dtSparePt->price;
				if($harga!=$hargaBarang){
					$newHarga=($hargaBarang+$harga)/2;
					
						$spareprt_price = array(
							"price" => $newHarga, 
						);

						$update_mst_s= $this->Pengeluaran_model->update_sparepart($spareprt_price, $kode);
				}
				$jmlBarang=$quantity;
				$saldoBarangPembelian=$jmlBarangDBPembelian+$jmlBarang;//Saldo Perlengkapan
				...
				$spareprt_item_data = array(
					...
					"kuantitas_pembelian" => $saldoBarangPembelian, 
				);
    ...
    private function _make_item_row($data) {
        ...
        if($val->status != "paid" AND $val->is_verified == "0"){
            return array(
                modal_anchor(get_uri("purchase/p_invoices/item_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_invoice'), "data-post-id" => $data->id))
    ...
    function send_invoice_modal_form($invoice_id) {
        ...
        if ($invoice_id) {
            ...
            $email_template = $this->Email_templates_model->get_final_template("send_purchase");
