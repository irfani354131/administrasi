...
class Request_holding extends MY_Controller {

    function __construct() {
        parent::__construct();

        $this->load->model('purchase/Purchase_Request_holding_model');
        
    }

    function index() {

        $this->template->rander("request_holding/index");
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Purchase_Request_holding_model->get_one($this->input->post('id'));
        $view_data['taxes_dropdown'] = array("" => "-") + $this->Taxes_model->get_dropdown_list(array("title"));
        
        $view_data['vendor_dropdown'] = array("" => "-") + $this->Master_Vendors_model->get_dropdown_list(array("name"));

        $this->load->view('request_holding/modal_form',$view_data);
    }
    ...
    function modal_form_edit() {
        ...
        $view_data['model_info'] = $this->Purchase_Request_holding_model->get_details($options)->row();
         $view_data['vendor_dropdown'] = array("" => "-") + $this->Master_Vendors_model->get_dropdown_list(array("name"));

        

        $this->load->view('request_holding/modal_form_edit', $view_data);
    }
    ...
    function add() {
        ...
        $save_id = $this->Purchase_Request_holding_model->save($data);
        if ($save_id) {
     ...
     function simpan() {
        ...
        $save_id = $this->Purchase_Request_holding_model->simpan($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Purchase_Request_holding_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
            if ($this->Purchase_Request_holding_model->delete($id)) {
    ...
    /* approval or undo a client */

     function approval($id){  
            $this->Purchase_Request_holding_model->approval($id);
            redirect(base_url().'purchase/request_holding');
        }
    ...
    function list_data() {
        ...
        $list_data = $this->Purchase_Request_holding_model->get_details()->result();
        $result = array();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Purchase_Request_holding_model->get_details($options)->row();
    ...
    private function _make_row($data) {
        ...
        $value = $this->Purchase_Request_holding_model->get_request_total_summary($data->id);
     ...
     if($data->status != "paid" AND $data->is_verified == "0"){
        $row_data[] = modal_anchor(get_uri("purchase/request_holding/modal_form_edit"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => 'Edit Request Pembelian', "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_client'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("purchase/request_holding/delete"), "data-action" => "delete"));
    }
        $row_data[] = "<a href='#''><i class='fa fa-check'></i></a>";
    ...
    private function _get_quotation_status_label($invoice_info, $return_html = true) {
        ...
        else if ($invoice_info->status == "approval") {
            $invoice_status_class   = "label-success";
            $status = "Disetujui";
    ...
    function item_modal_form() {
        ...
        $view_data['model_info'] = $this->Purchase_RequestItems_holding_model->get_one($this->input->post('id'));
        if (!$invoice_id) {
    ...
    function save_item() {
        ...
        $invoice_item_id = $this->Purchase_RequestItems_holding_model->save($invoice_item_data, $id);
        if ($invoice_item_id) {
            ...
            $item_info = $this->Purchase_RequestItems_holding_model->get_details($options)->row();
    ...
    function delete_item() {
        ...
        $id = $this->input->post('id');
        if ($this->input->post('undo')) {
            if ($this->Purchase_RequestItems_holding_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Purchase_RequestItems_holding_model->get_details($options)->row();
            ...
            if ($this->Purchase_RequestItems_holding_model->delete($id)) {
                $item_info = $this->Purchase_RequestItems_holding_model->get_one($id);
    ...
    function item_list_data($invoice_id = 0) {
        ...
        $list_data = $this->Purchase_RequestItems_holding_model->get_details(array("fid_quotation" => $invoice_id))->result();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("purchase/request_holding/item_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_invoice'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("purchase/request_holding/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function get_item_suggestion() {
        ...
        $items = $this->Purchase_RequestItems_holding_model->get_item_suggestion($key);
    ...
    function get_item_info_suggestion() {
        $item = $this->Purchase_RequestItems_holding_model->get_item_info_suggestion($this->input->post("item_name"));
        if ($item) {
    ...
    private function _get_invoice_total_view($invoice_id = 0) {
        $view_data["invoice_total_summary"] = $this->Purchase_Request_holding_model->get_request_total_summary($invoice_id);
        return $this->load->view('request/request_total_section', $view_data, true);
    }

    function get_invoice_status_bar($invoice_id = 0) {

        $view_data["invoice_info"] = $this->Purchase_Request_holding_model->get_details(array("id" => $invoice_id))->row();
    ...
    function send_request_modal_form($invoice_id) {
        ...
        if ($invoice_id) {
            ...
            $invoice_info = $this->Purchase_Request_holding_model->get_details($options)->row();
            ...
            $invoice_total_summary = $this->Purchase_Request_holding_model->get_request_total_summary($invoice_id);
    ...
    function send_request() {
        ...
        if (send_app_mail($contact->email, $subject, $message, array("attachments" => array(array("file_path" => $attachement_url)), "cc" => $cc, "bcc" => $bcc_emails))) {
            ...
            if ($this->Purchase_Request_holding_model->save($status_data, $invoice_id)) {
