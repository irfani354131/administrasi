...
class P_pengiriman extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("payments/index_dikirim");
    }
    ...
    function modal_form_kirim() {
        ...
        $this->load->view('payments/modal_form_kirim',$view_data);
    }
    ...
    function save_kirim() {
        ...
        $data = array(
            ...
            "tgl_dikirim" => $this->input->post('tgl_dikirim'),
            "keterangan" => $this->input->post('keterangan')
        );


        $save_id = $this->Purchase_Payments_model->save($data, $id);
        if ($save_id) {
    ...
    /* diterima or undo a client */

     function diterima($id){  
            $this->Purchase_Invoices_model->diterima($id);
            redirect(base_url().'purchase/p_penerimaan');
        }
    ...
    function list_data_receipt($id) {
        ...
        $list_data = $this->Purchase_Payments_model->getInvoicesCus($id)->result();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _make_row($data) {
        ...
        $dikirimDate = $data->tgl_dikirim;
        $newDate = date("d-M-Y", strtotime($originalDate));
        $newKirimDate = date("d-M-Y", strtotime($dikirimDate));
        if ($diterimaDate == "000-00-00") {
            echo "-";
        }
        else {
        $receiveDate = date("d-M-Y", strtotime($diterimaDate));
        }
        // $value = $this->Purchase_Payments_model->get_payments_total_summary($data->id);
        $row_data = array(
            anchor(get_uri("purchase/p_pengiriman/prints/" . $data->id."/".str_replace("/", "-", $data->code)), "#".$data->code),
            // $data->fid_inv,
            //"#".$data->code,
        ...
        $row_data[] = modal_anchor(get_uri("purchase/p_pengiriman/modal_form_kirim"), "<i class='fa fa-truck'></i>", array("class" => "edit", "title" => "Barang Dikirim", "data-post-id" => $data->id))
            .anchor("https://sia.dillahgroup.com/purchase/p_pengiriman/prints/".$data->id."/".str_replace("/", "-", $data->code), "<i class='fa fa-eye'></i>", array("class" => "view", "title" => lang('view'), "data-post-id" => $data->id));
    ...
    function view($id= 0){
        ...
        if ($id) {
            $view_data = get_p_payment_making_data($id);
