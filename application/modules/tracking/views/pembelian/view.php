        ...
        <div class="page-title clearfix">
            ...
            <div class="title-button-group">
                <div class="btn-group" role="group">
                  <?php
                    echo anchor(get_uri("tracking/t_pembelian"), "<i class='fa fa-arrow-left'></i> " . "Kembali", array("class" => "btn btn-primary", "title" => "Kembali"));
    ...
    <div class="content">
      ...
      <p><?php
           if ($request->status == 'approval')
             {
                $status = 'Request pembelian dengan code <strong>#'.$request->code.'</strong> Telah di verifikasi';
            }
        if ($request->status == 'draft')
            {
                $status = 'Pihak staff logistik melakukan request pembelian dan menunggu verifikasi oleh staff verifikasi';
            }
             if ($request->status == 'ditolak')
             {
                $status = 'Request pembelian dengan code <strong>#'.$request->code.'</strong> Ditolak';
            }
        ...
        if ($pembelian->status == 'terverifikasi')
            {
                 ...
                 $newDate = date("d M Y", strtotime($originalDate));
                 $status = 'Telah dilakukan Pemesanan pembelian oleh manajer keuangan dengan code PO <strong>#'.$pembelian->code.'</strong>';
        ...
        if ($bayar->paid == 'PAID')
            {
                 ...
                 $newDate = date("d M Y", strtotime($originalDate));
                 $status = 'Telah dibayar PO <strong>#'.$bayar->code.'</strong> oleh manajer keuangan.';
