...
class T_pembelian_holding extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("pembelian_holding/index");
    }
    ...
    /* approval or undo a client */
     ...
     function approval($id){  
            $this->Purchase_Request_model->approval($id);
            redirect(base_url().'purchase/request');
        }
   ...
   function list_data() {
    ...
    private function _make_row($data) {
        ...
        $row_data = array(
         ...
         $row_data[] = anchor(get_uri("tracking/t_pembelian_holding/view/").$data->id, "<i class='fa fa-eye'></i>", array("class" => "view", "title" => 'Tracking', "data-post-id" => $data->id));
    ...
    function view($id= 1){
        ...
        if ($id) {
            $view_data = get_request_making_data_holding($id);

            if ($view_data) {

                $view_data["request"] = $this->Purchase_Request_holding_model->get_request(array("id" => $id))->row();
                $view_data["pembelian"] = $this->Purchase_Request_holding_model->get_pembelian(array("id" => $id))->row();
                $view_data["bayar"] = $this->Purchase_Request_holding_model->get_pembayaran(array("id" => $id))->row();

                $this->template->rander("pembelian_holding/view", $view_data);
            } else {
                show_404();
            }
        }else{
