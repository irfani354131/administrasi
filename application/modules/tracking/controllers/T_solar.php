...
class T_solar extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("solar/index");
    }
    ...
    function view($id= 0){
        ...
        if ($id) {
            ...
            if ($view_data) {
                ...
                $view_data["solar"] = $this->Master_Sparepart_model->get_solar(array("id" => $id))->row();
                $view_data["keluar"] = $this->Master_Sparepart_model->get_solar_keluar(array("id" => $id))->row();
                //$view_data["bayar"] = $this->Sales_Invoices_model->get_pembayaran(array("id" => $id))->row();

                $this->template->rander("solar/view", $view_data);
            } else {
                show_404();
            }
        }else{
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            $data->title,
            $data->deskripsi,
            anchor(get_uri("tracking/t_solar/view/").$data->id, "<i class='fa fa-eye'></i>", array("class" => "view", "title" => 'Tracking', "data-post-id" => $data->id))
        );
    }
