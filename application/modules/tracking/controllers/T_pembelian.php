...
class T_pembelian extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("pembelian/index");
    }
    ...
    private function _make_row($data) {
        ...
        if ($data->status == 'approval')
            {
                $status = 'Request pembelian Telah di approve oleh manajer keuangan';
            }
        if ($data->status == 'draft')
            {
                $status = 'Request pembelian ke manajer keuangan';
            }


        $row_data = array(
        
            anchor(get_uri("purchase/request/view/" . $data->id), "#".$data->code),
            $newDate,
         ...
         $row_data[] = anchor(get_uri("tracking/t_pembelian/view/").$data->id, "<i class='fa fa-eye'></i>", array("class" => "view", "title" => 'Tracking', "data-post-id" => $data->id));
        return $row_data;
    }
    ...
    function view($id= 1){
        ...
        if ($id) {
            ...
            if ($view_data) {
                ...
                $view_data["request"] = $this->Purchase_Request_model->get_request(array("id" => $id))->row();
                $view_data["pembelian"] = $this->Purchase_Request_model->get_pembelian(array("id" => $id))->row();
                $view_data["bayar"] = $this->Purchase_Request_model->get_pembayaran(array("id" => $id))->row();

                $this->template->rander("pembelian/view", $view_data);
            } else {
                show_404();
            }
        }else{
