...
class T_Oli extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("oli/index");
    }
    ...
    function view($id= 0){
        ...
        if ($id) {
            ...
            if ($view_data) {
                ...
                $view_data["oli"] = $this->Master_Sparepart_model->get_oli(array("id" => $id))->row();
                $view_data["keluar"] = $this->Master_Sparepart_model->get_oli_keluar(array("id" => $id))->row();
                //$view_data["bayar"] = $this->Sales_Invoices_model->get_pembayaran(array("id" => $id))->row();

                $this->template->rander("oli/view", $view_data);
            } else {
                show_404();
            }
        }else{
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            $data->title,
            $data->deskripsi,
            anchor(get_uri("tracking/t_oli/view/").$data->id, "<i class='fa fa-eye'></i>", array("class" => "view", "title" => 'Tracking', "data-post-id" => $data->id))
        );
    }
