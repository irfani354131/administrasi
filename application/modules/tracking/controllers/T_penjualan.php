...
class T_penjualan extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("penjualan/index");
    }
    ...
    private function _make_row($data) {
        ...
        $row_data[] = anchor(get_uri("tracking/t_penjualan/view/").$data->id, "<i class='fa fa-eye'></i>", array("class" => "view", "title" => 'Tracking', "data-post-id" => $data->id));
   ...
   function view($id= 0){
        ...
        if ($id) {
            ...
            if ($view_data) {
                ...
                $view_data["invoices"] = $this->Sales_Invoices_model->get_invoices(array("id" => $id))->row();
                $view_data["bayar"] = $this->Sales_Invoices_model->get_pembayaran(array("id" => $id))->row();

                $this->template->rander("penjualan/view", $view_data);
            } else {
                show_404();
            }
        }else{
