...
if (!function_exists('get_team_member_profile_link')) {
    ...
    function get_team_member_profile_link($id = 0, $name = "", $attributes = array()) {
        $ci = get_instance();
        if ($ci->login_user->user_type === "staff" OR $ci->login_user->user_type === "sdm") {
...
if (!function_exists('get_p_invoices_making_data2')) {

    function get_p_invoices_making_data2($invoice_id) {
        $ci = get_instance();
        ...
        if ($invoice_info) {
            ...
            $data['client_info'] = $ci->Master_Vendors_model->get_one($data['invoice_info']->fid_cust);
            $data['invoice_items'] = $ci->Purchase_InvoicesItems_model->get_details(array("fid_invoices" => $invoice_id))->result();
            // $data['invoice_status_label'] = get_invoices_status_label($invoice_info);
...
if (!function_exists('get_request_making_data_holding')) {

    function get_request_making_data_holding($quotation_id) {
        $ci = get_instance();
        $quot_id = $ci->Purchase_Request_holding_model->get_details(array("id" => $quotation_id))->row();
        if ($quot_id) {
           
            $data["request"] = $ci->Purchase_Request_holding_model->get_request(array("id" => $id))->row();
            $data["pembelian"] = $ci->Purchase_Request_holding_model->get_pembelian(array("id" => $id))->row();
            $data["bayar"] = $ci->Purchase_Request_holding_model->get_pembayaran(array("id" => $id))->row();
...
if (!function_exists('get_t_sparepart_making_data')) {

    function get_t_sparepart_making_data($invoice_id) {
        $ci = get_instance();
        $invoice_info = $ci->Master_Sparepart_model->get_details(array("id" => $id))->row();
        if ($invoice_info) {
            //$data['invoice_info'] = $invoice_info;
            $data['sparepart_info'] = $ci->Master_Sparepart_model->get_details(array("id" => $data['sparepart_info']->id))->row();
...
if(!function_exists('getMaxId')){
    ...
    function getMaxId($table,$prefix = ''){
        ...
        $urut++;
        $data = sprintf("%03s",$urut).'/SIA/'.$prefix.'/'.date('ymd');
        return $data;
    }
}


if(!function_exists('getMaxIdHolding')){

    function getMaxIdHolding($table,$prefix = ''){
        
        $ci = get_instance();
        $dbholding = $ci->load->database('anotherdb', TRUE);

        $query = $dbholding->query("SELECT MAX(id) AS max_code from $table ")->row();
...
function getTambangInfo($id){

    $ci = get_instance();

    $ci->db->where("id",$id);

    $data = $ci->db->get('master_tambang')->row();
...
//Additional Produksi
if (!function_exists('get_detail_produksi')) {

    function get_detail_produksi($id) {
        $ci = get_instance();
        $quot_id = $ci->Produksi_model->get_details(array("id" => $id))->row();
        if ($quot_id) {
            $data['data_info'] = $quot_id;
            //$data['quot_info'] = $ci->Sales_Quotation_model->get_details(array("id" => $data['invoice_info']->fid_quot))->row();
            $data['material_items'] = $ci->Produksi_model->produksi_bahan(array('pb_produksi_id'=>$id))->result();
            $data['output_items'] = $ci->Produksi_model->produksi_barangjadi(array('bj_produksi_id'=>$id))->result();
            //$data['client_info'] = $ci->Master_Customers_model->get_one($data['invoice_info']->fid_cust);
            //$data['invoice_status_label'] = get_order_status_label($order_id);
            //$data["invoice_total_summary"] = $ci->Sales_Order_model->get_order_total_summary($order_id);
...
//Additional Produksi
if (!function_exists('get_detail_pengeluaran')) {

    function get_detail_pengeluaran($id) {
        $ci = get_instance();
        $quot_id = $ci->Pengeluaran_model->get_details(array("id" => $id))->row();
        if ($quot_id) {
            $data['data_info'] = $quot_id;
            //$data['quot_info'] = $ci->Sales_Quotation_model->get_details(array("id" => $data['invoice_info']->fid_quot))->row();
           // $data['material_items'] = $ci->Pengeluaran_model->produksi_bahan(array('pb_produksi_id'=>$id))->result();
            $data['output_items'] = $ci->Pengeluaran_model->pengeluaran_barang(array('id'=>$id))->result();
            //$data['client_info'] = $ci->Master_Customers_model->get_one($data['invoice_info']->fid_cust);
            //$data['invoice_status_label'] = get_order_status_label($order_id);
            //$data["invoice_total_summary"] = $ci->Sales_Order_model->get_order_total_summary($order_id);
...
//Additional Salary
if (!function_exists('get_detail_salary')) {

    function get_detail_salary($id) {
        $ci = get_instance();
        $quot_id = $ci->Salary_model->get_details(array("id" => $id))->row();
        if ($quot_id) {
            $data['data_info'] = $quot_id;
            //$data['quot_info'] = $ci->Sales_Quotation_model->get_details(array("id" => $data['invoice_info']->fid_quot))->row();
           // $data['material_items'] = $ci->Pengeluaran_model->produksi_bahan(array('pb_produksi_id'=>$id))->result();
